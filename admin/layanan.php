<?php
// Pastikan file konfigurasi/koneksi database utama sudah di-include di sini
// require_once '../config.php'; // Contoh, sesuaikan dengan struktur folder Anda
...
if ($action === 'delete' && $id) {
    ...
    if ($layanan) {
        // Hapus file gambar fisik jika ada (Opsional, tapi disarankan)
    ...
    if (isset($_FILES['gambar_path']) && $_FILES['gambar_path']['error'] == 0) {
        ...
        if (in_array(strtolower($file_extension), $allowed_ext)) {
            ...
            if (move_uploaded_file($_FILES["gambar_path"]["tmp_name"], $target_file)) {
                // Hapus gambar lama jika ada update gambar baru (Opsional)
...
$total_records = countRows($count_query, $params); // Asumsi fungsi ini ada
...
<div class="flex h-screen overflow-hidden">
    ...
    <div class="flex-1 flex flex-col min-w-0 overflow-hidden bg-[#f8f9fc]">

            
            <div class="mb-6 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
                <div>
                    <h2 class="text-lg font-semibold text-blue-900"><i class="fa-solid fa-headset mr-2"></i></i>Daftar Layanan</h2>
